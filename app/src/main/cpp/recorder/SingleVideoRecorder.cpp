...
//    LOGCATE("SingleVideoRecorder::StartH264EncoderThread start");
    //停止编码且队列为空时退出循环
    while (!recorder->m_exit || !recorder->frameQueue.Empty()) {
        ...
        LOGCATE("%s %s srcPixFmt %d,codecContext->pix_fmt %d",__FILE__,__FUNCTION__ ,srcPixFmt,AV_PIX_FMT_YUV420P);
        ...
        if (srcPixFmt != AV_PIX_FMT_YUV420P) {
            ...
            if (recorder->m_SwsContext != nullptr) {
                int slice = sws_scale(recorder->m_SwsContext, pImage->inData, pImage->inSize, 0,
...
int SingleVideoRecorder::OnFrame2Encode(NativeImage *nativeImage) {
    if (m_exit) return 0;
//    LOGCATE("SingleVideoRecorder::OnFrame2Encode [w,h,format]=[%d,%d,%d]", nativeImage->width,
//            nativeImage->height, nativeImage->format);
    NativeImage *pImage = new NativeImage();
    pImage->width = nativeImage->width;
    pImage->height = nativeImage->height;
    pImage->format = nativeImage->format;
    NativeImageUtil::AllocNativeImage(pImage);
    NativeImageUtil::CopyNativeImage(nativeImage, pImage);
